...
function updateMotionValuesFromProps(element, next, prev) {
  ...
  for (const key in next) {
    ...
    if (isMotionValue(nextValue)) {
      ...
      if (false) {
        warnOnce(nextValue.version === "10.12.6", `Attempting to mix Framer Motion versions ${nextValue.version} with 10.12.6 may not work as expected.`);
      }
...
var AnimatePresence = ({ children, custom, initial = true, onExitComplete, exitBeforeEnter, presenceAffectsLayout = true, mode = "sync" }) => {
  ...
  const forceRenderLayoutGroup = useContext7(LayoutGroupContext).forceRender;
...
//# sourceMappingURL=chunk-GYNFXYJN.js.map
